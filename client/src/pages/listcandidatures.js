...
const CandidatureList = () => {
      ...
      const notificationPromises = candidatures
        ...
        .map(async (candidature) => {
          ...
          if (formation.type === 'en_ligne') {
            message = message + ` Voici le lien de la formation : ${formation.url} et le code d'accès :${formation.code_access}`;
          } else if (formation.type === 'presentiel') {
            message = message + ` La formation aura lieu à l'adresse suivante : ${formation.adresse}. Les documents demandés sont les suivants : ${formation.Documents_demandes}.`;
          }
  ...
  <button className="custom-btn" onClick={handleShowFormationForm}>
    Organiser liste des formations
  </button>
)}
